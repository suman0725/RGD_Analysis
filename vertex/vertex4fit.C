...
void vertex4fit() {
    ...
    for(int i = 0; i < 6; ++i) {
        ...
        fitFunc[i] = new TF1(Form("fitFunc_S%d", i+1), "gaus(0)+gaus(3)", -15.0, 15.0); // Two Gaussian peaks for all sectors
    }
    ...
    for(int i = 0; i < 6; ++i) {
      ...
      fitFunc[i]->SetParameters(hvz[i]->GetMaximum(), -5.0, 1.0, hvz[i]->GetMaximum()/2, 5.0, 1.0);
