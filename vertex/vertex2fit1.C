...
void vertex2fit1() {
    ...
    for(int i = 0; i < 6; ++i) {
        ...
        fitFunc[i] = new TF1(Form("fitFunc_S%d", i+1), "gaus(0)+gaus(3)", -15.0, 15.0);
        fitFunc[i]->SetParameters(100, -5, 2, 100, 5, 2); // Initial guesses: amplitude, mean, sigma for two peaks
    }
    ...
    for(int i = 0; i < 6; ++i) {
       ...
       if (i != 4) { // Assume sector 5 (index 4) has only one peak
            hvz[i]->Fit(Form("fitFunc_S%d", i+1), "R");
        } else {

          TF1* singleGaussFit = new TF1(Form("singleGaussFit_S%d", i+1), "gaus", -15.0, 15.0);
            singleGaussFit->SetParameters(100, 0, 3); // Adjust these parameters based on your data
            hvz[i]->Fit(Form("singleGaussFit_S%d", i+1), "R");
        }
    }
}
